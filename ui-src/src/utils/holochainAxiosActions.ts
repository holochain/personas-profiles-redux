...
function makeBridgeCallPayload<PayloadType> (happ: string, zome: string, func: string, data: PayloadType): BridgeCallPayload<PayloadType> {
  return {
    request: {
      data: {
        happ,
...
export function createHolochainAsyncAction<paramType, returnType> (happ: string, zome: string, func: string) {
  ...
  newAction.create = (param: paramType) => action.request(makeBridgeCallPayload(happ, zome, func, param))
  return newAction
}
