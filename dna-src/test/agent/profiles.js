module.exports = scenario => {
  ...
  scenario('Can register a profile spec', async (s, t,  {alice}) => {
    const register_result = await alice.app.call("profiles", "register_app", {spec: testProfileSpec})
  ...
  scenario('Can get a list of profiles', async (s, t, {alice}) => {
    const register_result = await alice.app.call("profiles", "register_app", {spec: testProfileSpec})
    t.notEqual(register_result.Ok, undefined)
    const get_result = await alice.app.call("profiles", "get_profiles", {})
  ...
  scenario('Can register a profile spec', async (s, t,  {alice}) => {
    ...
    const map_result1 = await alice.app.call("profiles", "create_mapping",
      {
        mapping: {
          retrieverDna: "xxx",
    ...
    const result = await alice.app.call("personas", "create_persona", {spec: {name: "mapToPersona"}})
    const persona_address = result.Ok
    const add_result = await alice.app.call("personas", "add_field", {personaAddress: persona_address, field: {name: "test_field", data: "string data"}})

    // can callSync the function to create a mapping
    const map_result2 = await alice.app.call("profiles", "create_mapping",
      {
        mapping: {
          retrieverDna: "xxx",
